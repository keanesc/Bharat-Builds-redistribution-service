...
| Matching and responder ranking rules | FR-004 | Keane | Anirudh’s responder fixture shape and shared types | Partial | Ranking filters verified responders, capacity, food preference, and radius, then applies the documented remaining-time/distance behavior. | TODO: matching test |
...
| Lifecycle transitions and status history | FR-006 | Keane | Anirudh’s shared status types | Partial | Backend records actor/timestamp history for claim, pickup, delivery, cancellation, and any supported reassignment. | TODO: status response |
...
|---|---|---|---|---|---|
| CDK synthesis | Keane | Anirudh’s handler entrypoints | In progress | `npm run cdk:synth` / `npx cdk synth --strict` succeeds from a clean checkout. | TODO |
| Infrastructure diff review | Keane | Current API/table contract | Not started | `npm run cdk:diff` is reviewed for IAM, CORS, tables, indexes, logs, and destructive policies. | TODO |
...
| Deployed seed execution | Keane | Anirudh’s seed implementation and table outputs | Done | Deployed tables contain repeatable demo profiles and listings. | TODO: seed output |
...
| Security/configuration review | Keane | Current CDK and environment changes | Not started | No credentials are committed; demo identity is labelled non-production; CDK permissions and public CORS are reviewed. | TODO |
