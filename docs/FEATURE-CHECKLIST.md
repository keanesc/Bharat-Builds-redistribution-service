...
| Notification event generation and API behavior | FR-009 | Anirudh | Anirudh's shared event types; Keane's delivery channel contract | Done | Claim, cancellation, near-expiry, pickup, and failure events emit the documented payloads. | notifications.ts handler generates CLAIMED/CANCELLED/NEAR_EXPIRY/PICKED_UP/DELIVERY_FAILED payloads; 9 automated tests pass |
...
| Restaurant donation-history API | P1 | Anirudh | Anirudh's shared history types | Done | An authenticated/demo restaurant can retrieve its listing history and statuses. | restaurantId query param filter added to GET /listings returns restaurant's own listings |
...
| Admin approval data/rules | P1 | Anirudh | Anirudh's shared role/profile types | Done | Approval state and verification rules are represented in the backend or the seeded limitation is documented. | ApprovalStatus type added to shared types; approvalStatus optional field on ResponderProfile |
...
| API contract publication | Anirudh | Backend behavior | Done | `API.md`, shared types, handlers, and route/error examples agree before Nikhil integrates. | `docs/API.md` fully documents all routes, error codes, request/response shapes, and actor requirements |
