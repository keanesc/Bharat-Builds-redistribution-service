...
|---|---|---|---|---|---|---|
| Demo actor contract and header handling | FR-001 | Anirudh | None | Done | `X-Demo-Actor` is required where documented and demo actor IDs/types are shared; production authentication remains out of scope. | Actor validation added to POST /listings; DEMO_ACTOR_ROLES map in shared; X-Demo-Actor documented in API.md |
...
| Create-listing API validation and persistence | FR-002 | Anirudh | Anirudh's shared request types; Keane's DynamoDB primitives | Done | Valid requests persist an `AVAILABLE` listing with location, quantity, category, packed time, deadline, and status history. | foodCategory and packedAt validation added; 6 new POST validation tests pass |
...
| Available-listing query, radius, and expiry filtering | FR-003 | Anirudh | Anirudh’s shared listing types and fixtures; Keane’s DynamoDB primitives | Done | The API returns only available, non-expired listings within the requested radius and sorts by pickup deadline. | `listings.test.ts` covers query-time expiry filtering, radius filtering, foodCategory, minQuantity, and deadline sorting; all tests pass |
...
| Matching and responder ranking rules | FR-004 | Keane | Anirudh’s responder fixture shape and shared types | Done | Ranking filters verified responders, capacity, food preference, and radius, then applies the documented remaining-time/distance behavior. | `matching.test.ts` covers verification, capacity, preferences, exact radius, expiry, deterministic options, travel-window boundaries, defaults, and ranking order. |
...
| Atomic claim conditional update | FR-005 | Keane | Anirudh’s shared claim types | Done | Exactly one concurrent claim succeeds; a later claim returns `409 Conflict`. | `claims.test.ts` proves one simulated concurrent claim succeeds, the competitor receives the documented `409 CLAIM_CONFLICT`, and the conditional expression remains atomic. |
...
| Lifecycle transitions and status history | FR-006 | Keane | Anirudh’s shared status types | Done | Backend records actor/timestamp history for claim, pickup, delivery, cancellation, and any supported reassignment. | `claims.test.ts` and `status.test.ts` verify preserved history, one appended actor/timestamp event per successful transition, actor guards, and unchanged state after rejected transitions. |
...
| Query-time expiry filtering | FR-007 | Anirudh | Anirudh’s listing query; expiry fixtures | Done | Expired listings are excluded from availability queries and sorted results. | `listings.test.ts` covers `isExpired` boundary logic and GET /listings query-time expiry exclusion |
| Claim-time stale/expiry protection | FR-007 | Keane | Keane’s conditional-write path; Anirudh’s expiry contract and tests | Done | Expired listings cannot be claimed even when the frontend has stale data. | `claims.test.ts` verifies the strict `pickupDeadline > :now` condition and `409 CLAIM_CONFLICT` both at and after the deadline. |
...
| Notification event generation and API behavior | FR-009 | Anirudh | Anirudh's shared event types; Keane's delivery channel contract | Partial | Claim, cancellation, near-expiry, pickup, and failure events emit the documented payloads. | Event types and a handler contract exist with automated shape tests; lifecycle emission and an exposed CDK route are still pending. |
...
| Impact metrics API | FR-010 | Anirudh | Anirudh's shared metric types and fixtures; Keane's lifecycle responses | Done | `/dashboard/impact` returns listed, claimed, picked-up, delivered, expired, claim-time, and pickup-success metrics accurately. | Handler + 12 automated tests passing |
...
| Feature slice | PRD | Owner | Depends on | Status | Acceptance criteria | Evidence |
|---|---|---|---|---|---|---|
| Search/filter query contract | P1 | Anirudh | Anirudh's shared query types | Done | Supported neighbourhood, food, quantity, or time filters are documented and validated by the API. | foodCategory, minQuantity, maxMinutesUntilDeadline, restaurantId filters added to GET /listings; 3 filter tests pass |
...
| Restaurant donation-history API | P1 | Anirudh | Anirudh's shared history types | Partial | An authenticated/demo restaurant can retrieve its listing history and statuses. | `restaurantId` filtering exists, but the route does not yet bind the requested restaurant to the demo actor. |
...
| Responder capacity and profile data | P1 | Anirudh | Fixture format | Done | Seeded responder capacity and preferences remain typed, validated, and available to matching logic. | profiles.ts handler added (GET /profiles, GET /profiles/{id}); fixture verified by seed tests |
...
| Photo upload API/data support | P1 | Anirudh | Anirudh's shared photo field type; Keane's storage output | Partial | Listing contract accepts and persists the approved optional photo reference. | photoUrl optional field added to SurplusListing and CreateListingRequest shared types; awaiting Keane's S3 storage URL |
...
| Admin approval data/rules | P1 | Anirudh | Anirudh's shared role/profile types | Partial | Approval state and verification rules are represented in the backend or the seeded limitation is documented. | `ApprovalStatus` and the optional profile field exist; approval transition and authorization rules are not implemented. |
...
| Fixture and seed implementation | Anirudh | Shared types and fixture file | Done | `npm run seed` can load responders and listings with usable relative timestamps. | `dashboard.test.ts` validates fixture schema, profiles, and relative timestamp calculations; seed verified |
...
|---|---|---|---|---|---|
| Repository TypeScript validation | Anirudh | Nikhil and Keane workspace changes | Done | `npm run typecheck` passes for every workspace. | `npm run typecheck` passes across all 4 workspaces (frontend, backend, infra, shared) |
| Backend automated tests | Anirudh | Backend/shared implementation | Done | `npm test` covers claim conflicts, expiry, authorization, status history, filtering, dashboard calculations, and seed determinism. | `npm test` → 94 pass / 0 fail (matching, claims, status lifecycle, listings, dashboard, seed, notifications, health, profiles) |
...
| API contract publication | Anirudh | Backend behavior | Partial | `API.md`, shared types, handlers, and route/error examples agree before Nikhil integrates. | Active routes are documented; profile and notification sections are explicitly labelled handler-only until CDK wiring exists. |
